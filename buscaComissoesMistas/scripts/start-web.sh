...
# Use --fake-initial so existing SQLite schemas with pre-created indexes
# are treated as already migrated instead of erroring out when reapplying.
flock -x "${MIGRATION_LOCK}" python manage.py migrate --noinput --fake-initial
...
import django
django.setup()
from comissoes_mistas.models import Proposition
import sys
...
gunicorn busca_comissoes_mistas.wsgi:application \
  --bind 0.0.0.0:8080 \
