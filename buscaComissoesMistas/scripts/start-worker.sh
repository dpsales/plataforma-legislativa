...
if [ "${RUN_DB_MIGRATIONS:-0}" = "1" ]; then
  # Align with web container behaviour so repeated boots on existing
  # SQLite files do not fail due to already-created indexes.
...
CELERY_CMD=(
  celery -A busca_comissoes_mistas worker
