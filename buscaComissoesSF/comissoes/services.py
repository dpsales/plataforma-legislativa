...
TIPOS_INTERESSE = {"PL", "PEC", "PLP", "PLC", "PLS", "PDL", "PDS", "PLN", "EMC", "EMP", "PRN", "PES", "PRC"}
...
def fetch_commission_catalog(force: bool = False) -> dict[str, object]:
    """Retorna catálogo de comissões do Senado (incluindo mistas)."""
    now = timezone.now()
...
def _commission_siglas() -> list[str]:
    ...
    selecionadas = CommissionSelection.objects.filter(name=CommissionSelection.DEFAULT_NAME).first()
    if not selecionadas or not selecionadas.siglas:
        logger.warning("Nenhuma comissão selecionada; nenhuma matéria será coletada até que uma seleção seja configurada")
        return []

    validas = [sigla.strip().upper() for sigla in selecionadas.siglas if sigla]
    filtradas = [sigla for sigla in validas if sigla in vistos]
...
def fetch_dataset() -> pd.DataFrame:
    ...
    siglas_alvo = sorted(_commission_siglas())
    if not siglas_alvo:
        logger.info("Coleta interrompida: nenhuma comissão foi selecionada para acompanhamento")
        return pd.DataFrame()

    for comissao in siglas_alvo:
        if comissao == "PLEN":
